...
- Do not add `locals.tf` only to alias strings/composites for `main.tf` — put those expressions in `main.tf`.
...
- If `conditions.tf` does not exist in a new `base/*`, `foundation/*`, `project/*`, or `workload/*` stack, create it with this exact pattern:
  ```hcl
  locals {
