...
// mapRegionToShortCode maps AWS regions to short codes used in metadata
func mapRegionToShortCode(region string) string {
	...
	// Return mapped region if exists
	if shortCode, exists := regionMap[region]; exists {
		return shortCode
	}

	// Fallback: generate code based on region pattern
...
func (pg *ProjectGenerator) buildTemplateData(layer, env string) *models.TemplateData {
	...
	if env != "" {
		...
		// Special handling for organization layer
	...
	if useProfiles {
		...
		if profile != "" {
			for i := range defaultProviders {
				if defaultProviders[i].Name == "aws" {
...
// Note: organization layer is global and should not use this function
...
func (pg *ProjectGenerator) generateEnvironmentTable() string {
	...
	if len(pg.config.Environments) == 0 && !pg.isOrganizationLayerEnabled() {
