...
- **[terraform-aws-standard-platform](https://github.com/gocloudLa/terraform-aws-standard-platform)** — Platform GitHub repo: layers (`organization`, `security`, `backup`, `base`, `foundation`, `project`, `workload`), usage examples, and module docs
...
- **Generate directory structure** for Terraform/Terragrunt projects (organization, security, backup, base, foundation, project, workload).
...
- **Configure AWS SSO** profiles in `.aws/config` (one per environment plus `org`/`sec`/`bak` when enabled), and optionally verify GitHub organization membership via the `gh` CLI.
...
All `sso` subcommands accept `--provider aws|github|aws,github|all` to restrict which provider(s) they act on. By default (no `--provider`), a command acts on every provider declared in config: AWS is always available, GitHub only when `infrastructure.github_sso` is set (see **GitHub SSO control** in **Configuration file (gocloud.yaml)**). GitHub authentication goes through the `gh` CLI; gocloud never stores GitHub credentials itself.

#### `gocloud sso setup`

Writes `.aws/config` with one profile per environment that has `enable_sso` (default true), plus `{client}-org` when `infrastructure.organization.aws_account` is set, `{client}-sec` when `infrastructure.security.aws_account` is set, and `{client}-bak` when `infrastructure.backup.aws_account` is set. With the GitHub provider included, also creates `.gh` the same way — both get a nested `.gitignore` (`*`) so the generated state is never committed, and both scope `gocloud`'s own AWS/`gh` calls (`AWS_CONFIG_FILE` / `GH_CONFIG_DIR`) to the project directory instead of the shared `~/.aws`/`~/.config/gh`.

```bash
gocloud sso setup
gocloud sso setup --provider aws  # Restrict to the AWS provider only
```

#### `gocloud sso list`

Lists configured AWS SSO profiles from the generated `.aws/config`; also reports the logged-in GitHub account when the GitHub provider is included.

```bash
gocloud sso list
gocloud sso list --provider all  # AWS profiles + GitHub auth status
```

#### `gocloud sso login`

Login to AWS SSO profiles. Without flags it prompts interactively for a profile. With `--provider github`, runs GitHub's OAuth device flow directly (no `gh auth login` prompts: the browser opens immediately) and prints the `export GH_CONFIG_DIR=...` command needed for other tools (Terraform/terragrunt, or `gh` itself) run in the same shell to see that session — see **Terraform's `github` provider doesn't see my session** in **Troubleshooting**.
...
gocloud sso login --provider github     # OAuth device flow, no prompts
```

#### `gocloud sso verify`

Checks credential and account ID status (OK / Expired or Invalid / Account Mismatch). With the GitHub provider included, also reports GitHub auth status, token scopes, and — when `infrastructure.github_sso.organization` is set — organization membership. Status problems are reported the same way for both providers (printed, command still exits 0); only missing setup/config aborts the command.

```bash
gocloud sso verify
gocloud sso verify --provider aws,github
```
...
gocloud health check --environment bak  # backup account ({client}-bak) when configured
...
GoCloud uses a single YAML file (default `./gocloud.yaml`) to define your project: client name, AWS accounts and regions, layers to generate, backend (Terraform state), AWS SSO (login), providers, and secrets (SSM or SOPS). Every subsection below defines its own override scope; defaults are set at **Infrastructure** (global) and overridden in order **Environment → Project → Workload** (more specific wins). Special layers (`organization`, `security`, `backup`) are global and have their own override slot.
...
infrastructure:
  ...
  # organization, security, backup, layers, environment_order, enable_secrets,
...
**What it controls:** Activates a global `backup/` layer for the backup account (no per-environment subdirectory). Generation triggers when `infrastructure.backup.aws_account` is set (and `layers.backup` is not explicitly `false`). When activated, `gocloud sso setup` adds a `{client}-bak` profile and `gocloud generate` writes `backup/{main.tf,metadata.tf,backend.tf,providers.tf,_secrets.tf,terragrunt.hcl}` using the module `gocloudLa/standard-platform/aws//modules/backup`. The generated `backup/main.tf` declares the providers block `aws.org`, `aws.bkp`, `aws.bkv`, `aws.kms` — supply matching aliases via `infrastructure.backup.providers` or edit `main.tf` to wire your own aliases.

**Default behavior:**
- Inactive unless `aws_account` is set.
- Inherits global `backend`, `providers`, `aws_sso`, `metadata`, and `secrets`. Each can be overridden inside `infrastructure.backup`.
- `enable_secrets` for this layer inherits from the global `infrastructure.enable_secrets`.

```yaml
infrastructure:
  backup:
    aws_account: "123456789014"             # Required to activate the layer (SSO profile {client}-bak)
    metadata:
      support_team: "platform-bak"
    providers:
      default_providers:
        - name: "aws"
          region: "local.metadata.aws_region"
...
**What it controls:** Enables or disables generation of top-level layers. GoCloud generates **organization** (when `infrastructure.organization.aws_account` is set), **security** (when `infrastructure.security.aws_account` is set), **backup** (when `infrastructure.backup.aws_account` is set), **base**, **foundation**, plus **project**/**workload** from each environment.

**Default behavior:**
- `base`, `foundation`, `organization`, `security`, and `backup` are enabled by default.
- `organization`, `security`, and `backup` are only generated when their `aws_account` is configured.
- Per-environment `layers` may toggle `base` and `foundation` only; `organization`, `security`, and `backup` are global.
...
- More specific metadata (organization, security, backup, environment) overrides global keys per layer.
...
- `infrastructure.backup.metadata` for the `backup` layer
...
**What it controls:** Configures generation of `backend.tf` (Terraform state in S3 and state locking in DynamoDB). Resolved hierarchically at every scope (Global → Environment → Project / Workload). For special layers, see **Organization layer**, **Security layer**, and **Backup layer**.
...
**What it controls:** Configures generation of `providers.tf` (provider blocks, regions, aliases, optional `assume_role`, and automatic profile injection). Resolved hierarchically at every scope (Global → Environment → Project / Workload). For special layers, see **Organization layer**, **Security layer**, and **Backup layer**.
...
- Generated `main.tf` for `foundation` and `workload` layers wires `providers = { aws.use1 = aws.use1 }`; declare an `aws` provider with `alias: "use1"` if you want this wiring active. The `security` layer's `main.tf` wires `aws.org`, `aws.sec`, `aws.log`, `aws.kms`. The `backup` layer's `main.tf` wires `aws.org`, `aws.bkp`, `aws.bkv`, `aws.kms`. Declare those aliases or edit `main.tf`.
...
**What it controls:** The optional GitHub provider for `gocloud sso` (see **SSO** in **Commands**). Login runs GitHub's OAuth device flow, then stores the token via the `gh` CLI (`gh auth login --with-token` / `gh auth status`); gocloud never stores GitHub credentials itself. The consent screen requests `repo`, `read:org`, `gist`, and `admin:org` — the first three match `gh`'s own login minimum; `admin:org` is added so Terraform's `github` provider can manage organization resources (members, teams, settings). `organization` is the only YAML setting: when set, `gocloud sso verify` reports membership against that org.

**Default behavior:**
- `infrastructure.github_sso` is absent by default: the GitHub provider is not declared, and `--provider github` has nothing to act on.
- When `infrastructure.github_sso` is present, `organization` is required (there is nothing else to configure).
- Once GitHub auth itself is confirmed, organization mismatch and indeterminate membership (lookup failing, or a token missing `read:org`) are reported like AWS account-mismatch: printed, command still exits 0.
- A `gh auth status` failure or unparseable output only warns: the organization check is skipped entirely before it would ever run.

**Override scope:**

| Parameter | Global | Environment | Project | Workload |
|---|:---:|:---:|:---:|:---:|
| `github_sso.organization` | Y | N | N | N |

```yaml
infrastructure:
  github_sso:
    organization: "gocloudLa"
```
...
`gocloud sso login`/`verify --provider github` shells out to the GitHub CLI. Install it:

```bash
brew install gh   # macOS
# Linux: see https://github.com/cli/cli/blob/trunk/docs/install_linux.md
gocloud sso login --provider github
```

### Terraform's `github` provider doesn't see my session

`gocloud sso login --provider github` scopes its `gh` session to `.gh` in the project directory (see `sso setup`); other tools default to `~/.config/gh`, and `terraform-provider-github` only falls back to `gh auth token` when `legacy_client = true` is set on its `provider "github" {}` block.
